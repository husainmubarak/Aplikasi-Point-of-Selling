...
public class AplikasiGudangPOS extends JFrame {
    ...
    private void connectDatabase() {
        try {
            ...
            conn = DriverManager.getConnection(
                "jdbc:mysql://localhost:3306/pos_db", "root", "12345"
            );
        } catch (Exception e) {
    ...
    private void tambahProduk() {
        try {
            ...
            // Parse harga dengan handling format Indonesia
            ...
            if (kode.isEmpty() || nama.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Kode dan Nama produk harus diisi!");
                return;
            }
            
            if (harga <= 0 || stok < 0) {
                JOptionPane.showMessageDialog(this, "Harga harus > 0 dan Stok harus >= 0!");
                return;
            }
            ...
            if (e.getMessage().contains("Duplicate entry")) {
                JOptionPane.showMessageDialog(this, "Kode produk sudah ada! Gunakan kode yang berbeda.");
            } else {
                JOptionPane.showMessageDialog(this, "Error Database: " + e.getMessage());
            }
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(this, "Format angka tidak valid! Pastikan harga dan stok berupa angka.");
        }
    }
    ...
    private void prosesStok(String kode, String jumlahStr, String keterangan, String jenis, DefaultTableModel model) {
        try {
            if (kode.isEmpty() || jumlahStr.isEmpty()) {
                JOptionPane.showMessageDialog(this, "Kode produk dan jumlah harus diisi!");
                return;
            }
            ...
            if (jenis.equals("KELUAR")) {
                String sqlCek = "SELECT stok FROM produk WHERE kode_produk = ?";
            ...
            pstRiwayat.setString(4, keterangan.isEmpty() ? "-" : keterangan);
            pstRiwayat.executeUpdate();
            
            conn.commit();
            conn.setAutoCommit(true);
            
            JOptionPane.showMessageDialog(this, "Stok " + jenis.toLowerCase() + " berhasil diproses!");
            loadStok(model);
            txtJumlahStok.setText("");
            txtKeterangan.setText("");
            
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(this, "Format jumlah tidak valid! Harus berupa angka.");
        } catch (SQLException e) {
            try {
                conn.rollback();
